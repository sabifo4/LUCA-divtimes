...
To create the `LUCAdup_nosrp` file structure, we run the following commands from the PC before transferring them to the HPC:

```sh
# Run the following commands from 
# directory `00_CODEML`
mkdir -p HPC/LUCAdup_nosrp
cd HPC/LUCAdup_nosrp 
i=1
...
cd ../
rsync -avz --copy-links LUCAdup_nosrp <uname>@<server>:<path_to_your_wd_in_HPC>
```
...
```sh
# Run from `LUCAdup_nosrp/scripts` in the HPC.
...
```sh
# Run from `LUCAdup_nosrp/Hessian` dir on your local
...
```sh
# Run `MCMCtree` from
# `LUCAdup_nosrp/Hessian/1/prepare_codeml`
# dir on the HPC. 
# Please change directories until
# you are in there.
...
```sh
# Run from the `LUCAdup_nosrp/Hessian` dir on your local
# PC. Please change directories until
# you are there. Then, run the following
# commands.
...
```sh
# Run from `LUCAdup_nosrp` dir on your HPC. Please change directories until
# you are there. Then, run the following
# commands.
home_dir=$( pwd )
cd scripts
...
# Arg3: Name of the working directory (i.e., `LUCAdup_nosrp` in this analysis)
./generate_job_CODEML.sh $num_aln $home_dir/pipelines_Hessian LUCAdup_nosrp
```
...
```sh
# Run from `LUCAdup_nosrp/pipelines_Hessian` dir on your HPC.
# Please change directories until
# you are there. Then, run the following
# commands.
#
...
```sh
# Run from dir `LUCAdup_nosrp/Hessian/1` dir on your HPC
# Please change directories until
# you are there. Then, run the following
# commands.
...
mkdir out_CODEML
cd out_CODEML
rsync -avz --copy-links <uname>@<server>:<path_to_your_wd_in_HPC>/LUCAdup_nosrp/Hessian .
rsync -avz --copy-links <uname>@<server>:<path_to_your_wd_in_HPC>/LUCAdup_nosrp/pipelines_Hessian .

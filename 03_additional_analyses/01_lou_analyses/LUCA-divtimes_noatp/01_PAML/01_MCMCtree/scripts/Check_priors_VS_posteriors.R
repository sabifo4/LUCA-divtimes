...
# there are 241 taxa (241), so the number of internal nodes
# is `n_taxa-=241-1=240`.
...
# X will be an integer (i.e., 242). Subtract two to this number 
# (i.e., 242-2=240) and this will be your number of divergence times that are 
...
# GBM: The number of lines is 317068, and so you need to specify one less
# ILN: The number of lines is 319405, and so you need to specify one less
#
...
def_samples <- c( 120005, 317067, 319404 )
...
# approaches.
num_dirs     <- 3
paths_dat    <- c( paste( home_dir, "sum_analyses/00_prior/mcmc_files_CLK",
                          sep = "" ),
                   paste( home_dir, "sum_analyses/01_posterior/mcmc_files_GBM",
                          sep = "" ),
                   paste( home_dir, "sum_analyses/01_posterior/mcmc_files_ILN",
                          sep = "" )
)
...
names( dup_dat ) <- c( "LUCA", "TG-EUKARYA-ARCH", "LECA", "FUNGI", "METAZOA",
                       ...
                       "TG-EUKARYA-MITO" )
dup_dat[[1]] <- paste( "t_n", c(243,359), sep = "" )
dup_dat[[2]] <- paste( "t_n", c(263,447), sep = "" )
dup_dat[[3]] <- paste( "t_n", c(264,448), sep = "" )
dup_dat[[4]] <- paste( "t_n", c(267,327,386,451), sep = "" )
dup_dat[[5]] <- paste( "t_n", c(269,328,388,452), sep = "" )
dup_dat[[6]] <- paste( "t_n", c(270,329,389,453), sep = "" )
dup_dat[[7]] <- paste( "t_n", c(275,307,331,393,417,458), sep = "" )
dup_dat[[8]] <- paste( "t_n", c(278,309,333,395,421,461), sep = "" )
dup_dat[[9]] <- paste( "t_n", c(279,310,334,396,462), sep = "" )
dup_dat[[10]] <- paste( "t_n", c(287,404,470), sep = "" )
dup_dat[[11]] <- paste( "t_n", c(303,413), sep = "" )
dup_dat[[12]] <- paste( "t_n", c(304,414), sep = "" )
dup_dat[[13]] <-  paste( "t_n", c(324,382), sep = "" )
# Get objects with the labels in the csv file and the corresponding calib
# names
name_labs <- colnames( mcmc_obj$CLK$divt )
labs_in_csv <- paste( "t_n", calib_nodes$LUCAdup[,2], sep = "" )
# Start plotting!
for( i in 1:length(dup_dat) ){
  ...
  tmp_allB <- as.numeric( stringr::str_split( string = gsub( pattern = "B\\(|\\)", 
                                                             replacement = "",
                                                             x = calib_nodes$LUCAdup[tn_ind,3] ),
                                              pattern = "," )[[1]] )
  # Get max/min x and y
  ...
  tmp_name_calib     <- calib_nodes$LUCAdup[tmp_ind_name_calib,1]

...
dataset       <- c( "LUCAdup_noatp_CLK" )
perc          <- perc
def_samples   <- def_samples
prior         <- TRUE
out_dat       <- path_prior
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_noatp_prior" )
out_title_pdf <- c( "CLK")
th            <- 0.1
...
## [[ CLK ]] ####
sum_prior_QC$LUCAdup_noatp_CLK$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 120006    240
length( sum_prior_QC$LUCAdup_noatp_CLK$not_conv_nodes ) # 0
sum_prior_QC$LUCAdup_noatp_CLK$num_chains # 6

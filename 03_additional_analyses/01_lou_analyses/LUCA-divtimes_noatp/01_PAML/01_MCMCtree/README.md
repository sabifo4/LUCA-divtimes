...
```sh
# Run from `LUCAdup_noatp` dir on your HPC.
...
The `LUCAdup_noatp` directory will now have these two additional directories with the corresponding subdirectories:
...
rsync -avz --copy-links *MCMCtree*sh <uname>@<logdetails>:<path>/LUCAdup_noatp/scripts
```
...
```sh
# Run from the `LUCAdup_noatp` dir on your HPC.
# Please change directories until
# you are there. Then run the following
# commands.
home_dir=$( pwd )
...
# Arg 7: Name of working directory (e.g., `LUCAdup_noatp`)
# Arg 8: Boolean, enable duplication option? 0: no, 1: yes
# 
./generate_job_MCMCtree.sh $i GBM 1 $home_dir/pipelines_MCMCtree mcmctree4.10.7 $num_chains LUCAdup_noatp 1
./generate_job_MCMCtree.sh $i ILN 1 $home_dir/pipelines_MCMCtree mcmctree4.10.7 $num_chains LUCAdup_noatp 1
...
```sh
# Run everything from `LUCAdup_noatp` in your HPC
num_chains=6
...
rsync -avz --copy-links <uname>@<logdetails>:<path>/LUCAdup_noatp/tmp_to_transfer/00_prior .
rsync -avz --copy-link <uname>@<logdetails>:<path>/LUCAdup_noatp/pipelines_MCMCtree_prior .
...
dataset=$( echo CLK )
./Combine_MCMC.sh $dataset mcmc_files_CLK "`seq 1 6`" CLK 20000 Y noATP_CLK
```

The script above will generate a directory called `mcmc_files_CLK` inside the `00_prior` directory, where the `mcmc.txt` with the concatenated samples will be saved. In addition, a directory called `mcmcf4traces_noATP_CLK` will also be generated for each gene alignment so that formatted MCMC files compatible with programs such as `Tracer` can be used to check for chain convergence.
...
tt_dir=$( pwd )
name_tt=`ls LUCAdup_241sp_allcb_calib_MCMCtree.tree`
cd $ctl_dir
cd ../dummy_aln
aln_dir=$( pwd )
name_aln=`ls *aln`
...
cp $tt_dir/LUCAdup_241sp_uncalib.tree dummy_cal.tree
...
```sh
# Now, go to directory `LUCAdup_noatp/pipelines_MCMCtree/GBM` dir on your HPC
...
cd sum_analyses
# Now, trasnfer the data from the HPC
rsync -avz --copy-links <uname>@<logdetails>:<path>/LUCAdup_noatp/tmp_to_transfer/01_posterior .
rsync -avz --copy-links <uname>@<logdetails>:<path>/LUCAdup_noatp/pipelines_MCMCtree .
# Remove blank output files
...
dirname_1=GBM
dirname_2=ILN
./Combine_MCMC.sh $dirname_1 mcmc_files_GBM "`seq 1 16`" GBM 20000 Y noATP_GBM
./Combine_MCMC.sh $dirname_2 mcmc_files_ILN "`seq 1 16`" ILN 20000 Y noATP_ILN
```

Once the scripts above have finished, new directories called `mcmc_files_[GBM|ILN]` and `mcmcf4traces_noATP_[GBM|ILN]` will be created inside `01_posterior/`.

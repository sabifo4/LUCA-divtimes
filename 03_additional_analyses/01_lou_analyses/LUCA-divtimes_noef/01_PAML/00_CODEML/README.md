...
To create the `LUCAdup_noef` file structure, we run the following commands from the PC before transferring them to the HPC:

```sh
# Run the following commands from 
# directory `00_CODEML`
mkdir -p HPC/LUCAdup_noef
cd HPC/LUCAdup_noef 
i=1
...
```sh
# Run from `HPC/LUCAdup_noef`
# Copy alignment
cp ../../../../00_data_formatting/01_inp_data/LUCAdup_236sp_aln.phy alignments/1/
# Now, transfer the trees
cp ../../../../00_data_formatting/01_inp_data/LUCAdup_236sp_allcb_calib_MCMCtree.tree trees/calibrated/
cp ../../../../00_data_formatting/01_inp_data/LUCAdup_236sp_uncalib.tree trees/uncalibrated/
...
cd ../
rsync -avz --copy-links LUCAdup_noef <uname>@<server>:<path_to_your_wd_in_HPC>
```
...
```sh
# Run from `LUCAdup_noef/scripts` in the HPC.
...
```sh
# Run from `LUCAdup_noef/Hessian` dir on your local
...
```sh
# Run `MCMCtree` from
# `LUCAdup_noef/Hessian/1/prepare_codeml`
# dir on the HPC. 
# Please change directories until
# you are in there.
...
Reading seq #236: Volvox_carteri_mito_2
Sequences read..

1441 site patterns read, 1448 sites
Counting frequencies..

   221840 bytes for distance
   461120 bytes for conP
    46112 bytes for fhK
...
```sh
# Run from the `LUCAdup_noef/Hessian` dir on your local
# PC. Please change directories until
# you are there. Then, run the following
# commands.
...
```sh
# Run from `LUCAdup_noef` dir on your HPC. Please change directories until
# you are there. Then, run the following
# commands.
home_dir=$( pwd )
cd scripts
...
# Arg3: Name of the working directory (i.e., `LUCAdup_noef` in this analysis)
./generate_job_CODEML.sh $num_aln $home_dir/pipelines_Hessian LUCAdup_noef
```
...
```sh
# Run from `LUCAdup_noef/pipelines_Hessian` dir on your HPC.
# Please change directories until
# you are there. Then, run the following
# commands.
#
...
```sh
# Run from dir `LUCAdup_noef/Hessian/1` dir on your HPC
# Please change directories until
# you are there. Then, run the following
# commands.
...
mkdir out_CODEML
cd out_CODEML
rsync -avz --copy-links <uname>@<server>:<path_to_your_wd_in_HPC>/LUCAdup_noef/Hessian .
rsync -avz --copy-links <uname>@<server>:<path_to_your_wd_in_HPC>/LUCAdup_noef/pipelines_Hessian .

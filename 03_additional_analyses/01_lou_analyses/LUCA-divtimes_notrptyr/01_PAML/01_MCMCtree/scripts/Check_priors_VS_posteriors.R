...
# there are 244 taxa (244), so the number of internal nodes
# is `n_taxa-=244-1=243`.
...
# X will be an integer (i.e., 245). Subtract two to this number 
# (i.e., 244-2=244) and this will be your number of divergence times that are 
...
# GBM: The number of lines is 301595, and so you need to specify one less
# ILN: The number of lines is 313964, and so you need to specify one less
def_samples <- c( 120005, 301594, 313963 )
...
names( dup_dat ) <- c( "LUCA", "TG-EUKARYA-ARCH", "LECA", "FUNGI", "METAZOA",
                       ...
                       "TG-EUKARYA-MITO" )
dup_dat[[1]] <- paste( "t_n", c(246,365), sep = "" )
dup_dat[[2]] <- paste( "t_n", c(266,453), sep = "" )
dup_dat[[3]] <- paste( "t_n", c(267,454), sep = "" )
dup_dat[[4]] <- paste( "t_n", c(270,329,392,457), sep = "" )
dup_dat[[5]] <- paste( "t_n", c(272,330,394,459), sep = "" )
dup_dat[[6]] <- paste( "t_n", c(273,331,395,460), sep = "" )
dup_dat[[7]] <- paste( "t_n", c(278,309,335,399,423,465), sep = "" )
dup_dat[[8]] <- paste( "t_n", c(281,311,337,401,427,468), sep = "" )
dup_dat[[9]] <- paste( "t_n", c(282,312,338,402,469), sep = "" )
dup_dat[[10]] <- paste( "t_n", c(290,410,477), sep = "" )
dup_dat[[11]] <- paste( "t_n", c(305,419), sep = "" )
dup_dat[[12]] <- paste( "t_n", c(306,420), sep = "" )
dup_dat[[13]] <-  paste( "t_n", c(326,388), sep = "" )

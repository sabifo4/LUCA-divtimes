...
# Scan alignments
all_f <- list.files( path = "../../../../../00_data_formatting/00_raw_data/alignment/partitioned/", 
                     pattern = "fas$" ) # Get original `*trim..*fas` files
# Prepare output directory
if( ! dir.exists( "../00_raw_data/alignment/partitioned/" ) ){
  dir.create( "../00_raw_data/alignment/partitioned/" )
}
for( j in all_f ){
  cat( "[[ Analysing dataset ", j, " ]]\n")
  conc <- gsub( x = readLines( con = paste( "../../../../../00_data_formatting/00_raw_data/alignment/partitioned/",
                                            j, sep = "" ) ),
                pattern = ">", replacement = "" )
...
# Remove Try/Trp gene alignment
unlink( "../00_raw_data/alignment/partitioned/Tyr_filt.fasta" )

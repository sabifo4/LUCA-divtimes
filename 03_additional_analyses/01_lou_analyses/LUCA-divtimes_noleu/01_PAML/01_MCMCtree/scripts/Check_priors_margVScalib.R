...
# The number of lines is 120006, and so you need to specify one less here:
def_samples <- 120005
...
name_labs <- colnames( mcmc_priors$CLK$divt )
count <- 0
for( i in 1:length(dup_dat) ){
  pdf( file = paste( home_dir, "plots/dupnodes_", names(dup_dat)[i],
                     "_plot.pdf", sep = "" ), 
  ...
  max_x <- max( density( mcmc_priors$CLK$divt[[ nod_ind[1] ]], adj = 1 )$x ) + 0.2
  min_x <- min( density( mcmc_priors$CLK$divt[[ nod_ind[1] ]], adj = 1 )$x ) - 0.2
  plot( density( mcmc_priors$CLK$divt[[ nod_ind[1] ]], adj = 1 ),
        xlim = c(min_x,max_x),
  ...
  for( j in 2:tot_nodes ){
    count <- count + 1
    plot( density( mcmc_priors$CLK$divt[[ nod_ind[j] ]], adj = 1 ),
          xlim = c(min_x,max_x),
          main = paste( "Compare ages for ", names(dup_dat)[i], " - ",
                        dup_dat[[ i ]][j],
                        sep = "" ))

...
# there are 243 taxa (243), so the number of internal nodes
# is `n_taxa-=243-1=242`.
...
# X will be an integer (i.e., 244). Subtract two to this number 
# (i.e., 244-2=242) and this will be your number of divergence times that are 
...
# GBM: The number of lines is 309231, and so you need to specify one less
# ILN: The number of lines is 316717, and so you need to specify one less
#
...
def_samples <- c( 120005, 309230, 316716 )
...
names( dup_dat ) <- c( "LUCA", "TG-EUKARYA-ARCH", "LECA", "FUNGI", "METAZOA",
                       ...
                       "TG-EUKARYA-MITO" )
dup_dat[[1]] <- paste( "t_n", c(245,364), sep = "" )
dup_dat[[2]] <- paste( "t_n", c(265,451), sep = "" )
dup_dat[[3]] <- paste( "t_n", c(266,452), sep = "" )
dup_dat[[4]] <- paste( "t_n", c(269,328,391,455), sep = "" )
dup_dat[[5]] <- paste( "t_n", c(271,329,393,457), sep = "" )
dup_dat[[6]] <- paste( "t_n", c(272,330,394,458), sep = "" )
dup_dat[[7]] <- paste( "t_n", c(277,308,334,398,422,463), sep = "" )
dup_dat[[8]] <- paste( "t_n", c(279,310,336,400,425,465), sep = "" )
dup_dat[[9]] <- paste( "t_n", c(280,311,337,401,466), sep = "" )
dup_dat[[10]] <- paste( "t_n", c(288,409,474), sep = "" )
dup_dat[[11]] <- paste( "t_n", c(304,418), sep = "" )
dup_dat[[12]] <- paste( "t_n", c(305,419), sep = "" )
dup_dat[[13]] <-  paste( "t_n", c(325,387), sep = "" )

...
# GBM and ILN: The number of lines are the following:
#                 ATP--> 295945, 304994 --> get one less
#                 EF --> 289369, 306880 --> get one less
#                 Leu--> 305029, 315278 --> get one less
#                 SRP--> 296763, 301147 --> get one less
#                 Tyr--> 301955, 306000 --> get one less
#
...
def_samples <- c( c( 120005, 29594, 304993 ),  # ATP
                  c( 120005, 289368, 306879 ),  # EF
                  c( 120005, 305028, 315277 ),  # Leu
                  c( 120005, 296762, 301146 ),  # SRP
                  c( 120005, 301954, 305999 ) ) # Tyr
...
delcol_obj <- rep( c( 1, 2, 2 ), 5 ) # prior (1), posterior (2), posterior (2)
...
dat_ff <- list.files( path = "../calib_files", pattern = "margVScalib.csv",
                      full.names = FALSE )
...
names( mcmc_obj ) <- rep( c( "CLK", "GBM", "ILN" ), 5 )
prior <- rep( c( TRUE, FALSE, FALSE ), 5 )
count <- 0
names_out_mcmc <- gsub( pattern = "..*\\/", replacement = "", x = paths_dat )
...
for( k in 1:length(dat) ){
  mcmc_pos <- mcmc_pos + add
  cat( "MCMC pos for dat ", dat[k], ": ", mcmc_pos, "\n")
  for( i in 1:length(dup_dat) ){
    pdf( file = paste( home_dir, "plots/priorVSpost_", dat[k], "_", 
                       names(dup_dat)[i], "_plot.pdf",
                       sep = "" ), 
    ...
    tmp_allB <- as.numeric( stringr::str_split( string = gsub( pattern = "B\\(|\\)", 
                                                               replacement = "",
                                                               x = calib_nodes[[ 1 ]][tn_ind,3] ),
                                                pattern = "," )[[1]] )
    # Get max/min x and y
    ##> NOTE: Only the first number is used, because the other number/s in 
    ##> `dup_dat[[i]` have the same fossil info!
    tn_matrix  <- which( colnames(mcmc_obj[[ mcmc_pos[1] ]]$divt) %in% dup_dat[[ i ]] ) #CLK
    max_y <- max( c( max( density( mcmc_obj[[ mcmc_pos[2] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$y ),  #GBM
                     max( density( mcmc_obj[[ mcmc_pos[3] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$y ),  #ILN
                     max( density( mcmc_obj[[ mcmc_pos[1] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$y ) ) ) #CLK
    max_x <- max( c( max( density( mcmc_obj[[ mcmc_pos[2] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$x ),    #GBM
                     max( density( mcmc_obj[[ mcmc_pos[3] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$x ),    #ILN
                     max( density( mcmc_obj[[ mcmc_pos[1] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$x ),    #CLK
                     tmp_allB[2]) )
    min_x <- min( c( min( density( mcmc_obj[[ mcmc_pos[2] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$x ),
                     min( density( mcmc_obj[[ mcmc_pos[3] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$x ),
                     min( density( mcmc_obj[[ mcmc_pos[1] ]]$divt[[ tn_matrix[1] ]], adj = 1 )$x ),
                     tmp_allB[1] ) )
    # Get name of calibration, which corresponds to the first calibration
    # in the list
    tmp_ind_name_calib <- which( labs_in_csv %in% dup_dat[[ i ]][1] )
    tmp_name_calib     <- calib_nodes[[ 1 ]][tmp_ind_name_calib,1]
    ...
    for( j in dup_dat[[ i ]] ){
      ...
      tmp_tn         <- which( colnames(mcmc_obj[[ mcmc_pos[2] ]]$divt) %in% j )
      plot( density( mcmc_obj[[ mcmc_pos[2] ]]$divt[[ tmp_tn ]], adj = 1 ),
            main = paste( "Comparing densities (col",
                          tmp_tn, ") | ", tmp_name_calib, " (", dat[k], ")",
                          sep = "" ),
            col = "blue", ylim = c(0,max_y), xlim = c(min_x, max_x) )
      lines( density( mcmc_obj[[ mcmc_pos[1] ]]$divt[[ tmp_tn ]], adj = 1 ),
             col = "brown" )
      lines( density( mcmc_obj[[ mcmc_pos[3] ]]$divt[[ tmp_tn ]], adj = 1 ),
             col = "darkolivegreen3" )

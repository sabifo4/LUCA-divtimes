  ...
  if (plot.type == "distributions") {
    ...
    if( num.models > 1 ){ 
      ...
      if (length(density.border.col) < len.all.nd) 
        density.border.col <- rep(density.border.col, 
                                  len.all.nd)
      # SAC-240108: Get coords to later add text labels
      coords_k <- vector( mode = "list", len.all.nd )
      for ( k in 1:len.all.nd ) {
        ...
        yy <- y.scale + lower.y
        polygon(xx, yy, col = density.col[k], border = density.border.col[k])
        # SAC-240108: Add list with x and y values for "k"
        coords_k[[ k ]] <- list( x = xx, y = yy )
      }
                                    ...
                                    abs.age.line = -0.4, pos.age = NULL, ...) 
{
  ...
  if (add.time.scale) {
    ...
    #>> 240119-SAC Update period ages for Hadean and Eoarchean
    #>> All modifications below when `MCMCtreeR:::Period` appears have been
    #>> changed!
    # I need to create an additional "bin" for Hadean because, otherwise,
    # I cannot reach the limit (max.xlim is 4337.449, and so I need to 
    # extend Hadean until 4600 to allow for this bin to appear!)
    # Each bin signals the "starting point", not the end, that's why.
    # The last bin, pos 21, is the end of Hadean and pos 20 is the start of
    # Hadean
    period_mcmctreeR <- rep( 0, 21 )
    period_mcmctreeR[c(1:20)] <- MCMCtreeR:::Period
    names( period_mcmctreeR )[c(1:20)] <- names( MCMCtreeR:::Period )
    period_mcmctreeR[c(19,20)] <- c( 3600, 4031 )
    period_mcmctreeR[c(21)] <- 4600
    names( period_mcmctreeR )[21] <- "Hadean"
    # cat( "max.xlim:", max.xlim, "\ntweaked:", period_mcmctreeR,
    #      "original:",  MCMCtreeR:::Period, "\n" )
    #period.bin <- .bincode(max.xlim, MCMCtreeR:::Period)
    period.bin <- .bincode(max.xlim, period_mcmctreeR)
    ...
    bin.period <- max.xlim - c(period_mcmctreeR[1:period.bin], max.xlim)

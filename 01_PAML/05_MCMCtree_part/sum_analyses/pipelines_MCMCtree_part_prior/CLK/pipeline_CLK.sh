...
# --------------------------------------- #
# Creating file structure to run MCMCtree_part_prior #
# --------------------------------------- # 
...
cd $main_dir/MCMCtree_part_prior/$SGE_TASK_ID/CLK/$dir
home_dir=$( pwd )
...
exec 3>&1> >(while read line; do echo "$line" >> $pipeline_dir/log.MCMCtree_part_prior.dir$dir"_r"$SGE_TASK_ID".txt"; done;) 2>&1
start=`date`
...
# and run MCMCtree_part_prior
duplication=1
if [[ $duplication -eq 1 ]]
then 
printf "\nduplication = 1\n" >> $home_dir/mcmctree_$dir"_r"$SGE_TASK_ID".ctl"
printf "\nRunning MCMCtree_part_prior for divergence times estimation ... ...\n"
cd $home_dir
...
# 6. Close
printf "\n"
echo MCMCtree_part_prior FINISHED"!"
printf "\n"
end=`date`
echo Job ends":" $end

...
# --------------------------------------- #
# Creating file structure to run MCMCtree_part #
# --------------------------------------- # 
...
cd $main_dir/MCMCtree_part/$SGE_TASK_ID/GBM/$dir
home_dir=$( pwd )
...
exec 3>&1> >(while read line; do echo "$line" >> $pipeline_dir/log.MCMCtree_part.dir$dir"_r"$SGE_TASK_ID".txt"; done;) 2>&1
start=`date`
...
# and run MCMCtree_part
duplication=1
if [[ $duplication -eq 1 ]]
then 
printf "\nduplication = 1\n" >> $home_dir/mcmctree_$dir"_r"$SGE_TASK_ID".ctl"
printf "\nRunning MCMCtree_part for divergence times estimation ... ...\n"
cd $home_dir

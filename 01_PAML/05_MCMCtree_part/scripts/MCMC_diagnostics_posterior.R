...
delcol_post <- 10 # There are 10 columns: mu[1-5], sigma2_[1-5]
...
dataset       <- c( "LUCA_part_GBM", "LUCA_part_ILN" )
perc          <- perc
def_samples   <- def_samples
prior         <- FALSE
out_dat       <- all_paths
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_post_part_GBM",
                   "Convergence_plot_post_part_ILN" )
out_title_pdf <- c( "GBM","ILN" )
th            <- c(0.1,0.1)
# Generate a list to keep all the returned objects
sum_post_part_QC          <- vector( "list", length( all_paths ) )
names( sum_post_part_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_post_part_QC[[ i ]] <- QC_conv( num_dirs = num_dirs, delcol = delcol[i], 
...
save( file = paste( home_dir, "out_RData/sum_post_part_QC.RData", sep = "" ),
      sum_post_part_QC )

# Print out sum stats in the script as a log

## [GBM] ####
sum_post_part_QC$LUCA_part_GBM$ESS_results
# $median
# [1] 3581.5
# 
# $min
...
# $total_samples
# [1] 56203   245
length( sum_post_part_QC$LUCA_part_GBM$not_conv_nodes ) # 0
sum_post_part_QC$LUCA_part_GBM$num_chains # 16

## [[ ILN ]] ####
sum_post_part_QC$LUCA_part_ILN$ESS_results
# $median
# [1] 3709
# 
# $min
...
# $total_samples
# [1] 56889   245
length( sum_post_part_QC$LUCA_part_ILN$not_conv_nodes ) # 0
sum_post_part_QC$LUCA_part_ILN$num_chains # 16

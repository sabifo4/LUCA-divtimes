...
# (i.e., 247-2=246) and this will be your number of divergence times that are 
...
delcol_prior <- 5 # There are 5 columns: mu[1-5]
...
path          <- path_prior
num_divt      <- num_divt
node_calib    <- calib_nodes[[1]] # matrix needed! same for both datasets
dataset       <- c( "LUCAdup_part_CLK" )
perc          <- perc
def_samples   <- def_samples
prior         <- TRUE
out_dat       <- path_prior
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_part_prior" )
out_title_pdf <- c( "CLK")
th            <- 0.1
# Generate a list to keep all the returned objects
sum_prior_part_QC          <- vector( "list", length( path_prior ) )
names( sum_prior_part_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_prior_part_QC[[ i ]] <- QC_conv( num_dirs = num_dirs, delcol = delcol[i], 
...
save( file = paste( home_dir, "out_RData/sum_prior_part_QC.Rdata", sep = "" ),
      sum_prior_part_QC )

# Print out sum stats in the script as a log

## [[ STR-CONC-GBM ]] ####
sum_prior_part_QC$LUCAdup_part_CLK$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 120006    245
length( sum_prior_part_QC$LUCAdup_part_CLK$not_conv_nodes ) # 0
sum_prior_part_QC$LUCAdup_part_CLK$num_chains # 6

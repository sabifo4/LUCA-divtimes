...
delcol_prior <- 5 # There is five columns: mu[1-5]
...
dataset       <- c( "LUCAdup_part_fosscb_CLK" )
perc          <- perc
def_samples   <- def_samples
prior         <- TRUE
out_dat       <- path_prior
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_prior_part_fosscb" )
out_title_pdf <- c( "CLK")
th            <- 0.1
# Generate a list to keep all the returned objects
sum_prior_part_fosscb_QC          <- vector( "list", length( path_prior ) )
names( sum_prior_part_fosscb_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_prior_part_fosscb_QC[[ i ]] <- QC_conv( num_dirs = num_dirs,
                                              delcol = delcol[i], 
                                              path = path[i],
                                              num_divt = num_divt, 
...
save( file = paste( home_dir, "out_RData/sum_prior_part_fosscb_QC.RData",
                    sep = "" ),
      sum_prior_part_fosscb_QC )

# Print out sum stats in the script as a log

## [[ STR-CONC-CLK ]] ####
sum_prior_part_fosscb_QC$LUCAdup_part_fosscb_CLK$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 120006    245
length( sum_prior_part_fosscb_QC$LUCAdup_part_fosscb_CLK$not_conv_nodes ) # 0
sum_prior_part_fosscb_QC$LUCAdup_part_fosscb_CLK$num_chains # 6

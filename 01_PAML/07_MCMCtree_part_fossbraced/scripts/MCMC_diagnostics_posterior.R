...
delcol_post <- 10 # There are two column: mu[1-5], sigma2_[1-5]
...
# of chains we ran).
all_paths   <- c( paste( home_dir, "sum_analyses/01_posterior/GBM/",
                         sep = "" ),
...
path          <- all_paths
num_divt      <- num_divt
node_calib    <- calib_nodes[[ 1 ]] # matrix required, same for both datasets
dataset       <- c( "LUCAdup_part_fosscb_GBM", "LUCAdup_part_fosscb_ILN" )
perc          <- perc
def_samples   <- def_samples
prior         <- FALSE
out_dat       <- all_paths
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_post_part_fosscb_GBM",
                   "Convergence_plot_post_part_fosscb_ILN" )
out_title_pdf <- c( "GBM","ILN" )
th            <- c( 0.1, 0.1 )
# Generate a list to keep all the returned objects
sum_post_part_fosscb_QC          <- vector( "list", length( all_paths ) )
names( sum_post_part_fosscb_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_post_part_fosscb_QC[[ i ]] <- QC_conv( num_dirs = num_dirs,
                                             delcol = delcol[i], 
                                             path = path[i],
                                             num_divt = num_divt, 
...
save( file = paste( home_dir, "out_RData/sum_post_part_fosscb_QC.RData",
                    sep = "" ),
      sum_post_part_fosscb_QC )

# Print out sum stats in the script as a log

## [[ GBM | part | fosscb (cbB) ]] ####
sum_post_part_fosscb_QC$LUCAdup_part_fosscb_GBM$ESS_results
# $median
# [1] 2619.5
# 
# $min
...
# $total_samples
# [1] 38434   245
length( sum_post_part_fosscb_QC$LUCAdup_part_fosscb_GBM$not_conv_nodes ) # 0
sum_post_part_fosscb_QC$LUCAdup_part_fosscb_GBM$num_chains # 16

## [[ ILN | part | fosscb (cbB) ]] ####
sum_post_part_fosscb_QC$LUCAdup_part_fosscb_ILN$ESS_results
# $median
# [1] 2078.5
# 
# $min
...
# $total_samples
# [1] 37795   245
length( sum_post_part_fosscb_QC$LUCAdup_part_fosscb_ILN$not_conv_nodes ) # 0
sum_post_part_fosscb_QC$LUCAdup_part_fosscb_ILN$num_chains # 16

...
Now, before running `MCMCtree` to sample from the posterior, we will run `MCMCtree` but sampling from the prior (see [the previous `README.md` file for more details](../05_MCMCtree/README.md#1-setting-the-file-structure-to-run-mcmctree)). Firstly, we will generate a directory where `MCMCtree` will run when sampling from the prior:
...
> [!NOTE]
>
> Please note that we have not been able to upload to this GitHub repository the `mcmcf4traces_part_fosscb_CLK` directory nor the large concatenated `mcmc.txt` inside `mcmc_files_part_fosscb_CLK`. If you have not generated them by following our guidelines and/or want to check out results, please download [the archive we stored at the University of Bristol data repository (~71Gb)](https://data.bris.ac.uk/datasets/405xnm7ei36d2cj65nrirg3ip/LUCAdivtimes.zip), which follows the same file structure as in this repository.
...
Once the scripts above have finished, new directories called `mcmc_files_part_fosscb_[GBM|ILN]` and `mcmcf4traces_part_fosscb_[GBM|ILN]` will be created inside `01_posterior/`.

> [!NOTE]
>
> Please note that we have not been able to upload to this GitHub repository the `mcmcf4traces_part_fosscb_[GBM|ILN]` directory nor the large concatenated `mcmc.txt` inside `mcmc_files_part_fosscb_[GBM|ILN]`. If you have not generated them by following our guidelines and/or want to check out results, please download [the archive we stored at the University of Bristol data repository (~71Gb)](https://data.bris.ac.uk/datasets/405xnm7ei36d2cj65nrirg3ip/LUCAdivtimes.zip), which follows the same file structure as in this repository.

...
dataset       <- c( "LUCAdup_notcb_CLK" )
perc          <- perc
def_samples   <- def_samples
prior         <- TRUE
out_dat       <- path_prior
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_prior_notcb" )
out_title_pdf <- c( "CLK")
th            <- 0.1
# Generate a list to keep all the returned objects
sum_prior_notcb_QC          <- vector( "list", length( path_prior ) )
names( sum_prior_notcb_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_prior_notcb_QC[[ i ]] <- QC_conv( num_dirs = num_dirs, delcol = delcol[i], 
...
save( file = paste( home_dir, "out_RData/sum_prior_notcb_QC.Rdata", sep = "" ),
      sum_prior_notcb_QC )

# Print out sum stats in the script as a log

## [[ CLK -- notcb ]] ####
sum_prior_notcb_QC$LUCAdup_notcb$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 40002   245
length( sum_prior_notcb_QC$LUCAdup_notcb$not_conv_nodes ) # 0
sum_prior_notcb_QC$LUCAdup_notcb$num_chains # 2

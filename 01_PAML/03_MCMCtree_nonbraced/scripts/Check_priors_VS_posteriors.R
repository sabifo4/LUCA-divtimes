...
#
# CLK: The number of lines is 40002, and so you need to specify one less
# GBM: The number of lines is 221526, and so you need to specify one less
# ILN: The number of lines is 247184, and so you need to specify one less
#
...
def_samples <- c( 40001, 221525, 247183 )
...
paths_dat    <- c( paste( home_dir,
                          "sum_analyses/00_prior/mcmc_files_notcb_CLK",
                          sep = "" ),
                   paste( home_dir,
                          "sum_analyses/01_posterior/mcmc_files_notcb_GBM",
                          sep = "" ),
                   paste( home_dir,
                          "sum_analyses/01_posterior/mcmc_files_notcb_ILN",
                          sep = "" )
)
...
labs_in_csv <- paste( "t_n", calib_nodes$LUCAdup_notcb[,2], sep = "" )
# Start plotting!
for( i in 1:length(dup_dat) ){
  ...
  tmp_allB <- as.numeric( stringr::str_split( string = gsub( pattern = "B\\(|\\)", 
                                                             replacement = "",
                                                             x = calib_nodes$LUCAdup_notcb[tn_ind,3] ),
                                              pattern = "," )[[1]] )
  # Get max/min x and y
  ...
  tmp_name_calib     <- calib_nodes$LUCAdup_notcb[tmp_ind_name_calib,1]

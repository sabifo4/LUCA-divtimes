...
dataset       <- c( "LUCA_notcb_GBM", "LUCA_notcb_ILN" )
perc          <- perc
def_samples   <- def_samples
prior         <- FALSE
out_dat       <- all_paths
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_post_notcb_GBM",
                   "Convergence_plot_post_notcb_ILN" )
out_title_pdf <- c( "GBM","ILN" )
th            <- 0.1
# Generate a list to keep all the returned objects
sum_post_notcb_QC          <- vector( "list", length( all_paths ) )
names( sum_post_notcb_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_post_notcb_QC[[ i ]] <- QC_conv( num_dirs = num_dirs, delcol = delcol[i], 
...
save( file = paste( home_dir, "out_RData/sum_post_notcb_QC.RData", sep = "" ),
      sum_post_notcb_QC )

# Print out sum stats in the script as a log

## [[ GBM | conc | notcb ]] ####
sum_post_notcb_QC$LUCA_notcb_GBM$ESS_results
# $median
# [1] 14038.5
# 
# $min
...
# $total_samples
# [1] 221526    245
length( sum_post_notcb_QC$LUCA_notcb_GBM$not_conv_nodes ) # 0
sum_post_notcb_QC$LUCA_notcb_GBM$num_chains # 16

## [[ ILN | conc | notcb ]] ####
sum_post_notcb_QC$LUCA_notcb_ILN$ESS_results
# $median
# [1] 15443.5
# 
# $min
...
# $total_samples
# [1] 247184    245
length( sum_post_notcb_QC$LUCA_notcb_ILN$not_conv_nodes ) # 0
sum_post_notcb_QC$LUCA_notcb_ILN$num_chains # 16

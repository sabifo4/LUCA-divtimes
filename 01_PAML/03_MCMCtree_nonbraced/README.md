...
The MCMC diagnostics found 4 chains problematic (run script [`MCMC_diagnostics_prior.R`](scripts/MCMC_diagnostics_prior.R)). Therefore, we used an in-house bash script, [`Combine_MCMC.sh`](scripts/Combine_MCMC.sh), to concatenate the `mcmc.txt` files for the chains that passed the filters in a unique file.

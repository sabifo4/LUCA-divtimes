...
dataset       <- c( "LUCAdup_part_notcb_GBM","LUCAdup_part_notcb_ILN" )
perc          <- perc
def_samples   <- def_samples
prior         <- FALSE
out_dat       <- all_paths
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_post_part_notcb_GBM",
                   "Convergence_plot_post_part_notcb_ILN" )
out_title_pdf <- c( "GBM","ILN" )
th            <- 0.1
# Generate a list to keep all the returned objects
sum_post_part_notcb_QC          <- vector( "list", length( all_paths ) )
names( sum_post_part_notcb_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_post_part_notcb_QC[[ i ]] <- QC_conv( num_dirs = num_dirs,
                                            delcol = delcol[i], 
...
save( file = paste( home_dir, "out_RData/sum_post_part_notcb_QC.RData",
                    sep = "" ),
      sum_post_part_notcb_QC )

# Print out sum stats in the script as a log

## [GBM] ####
sum_post_part_notcb_QC$LUCAdup_part_notcb_GBM$ESS_results
# $median
# [1] 2934
# 
# $min
...
# $total_samples
# [1] 45649   245
length( sum_post_part_notcb_QC$LUCAdup_part_notcb_GBM$not_conv_nodes ) # 0
sum_post_part_notcb_QC$LUCAdup_part_notcb_GBM$num_chains # 13

## [[ ILN ]] ####
sum_post_part_notcb_QC$LUCAdup_part_notcb_ILN$ESS_results
# $median
# [1] 3090
# 
# $min
...
# $total_samples
# [1] 48216   245
length( sum_post_part_notcb_QC$LUCAdup_part_notcb_ILN$not_conv_nodes ) # 8
sum_post_part_notcb_QC$LUCAdup_part_notcb_ILN$num_chains # 14

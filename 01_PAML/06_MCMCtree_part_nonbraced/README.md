...
Now, before running `MCMCtree` to sample from the posterior, we will run `MCMCtree` but sampling from the prior (see [the previous `README.md` file for more details](../05_MCMCtree_part/README.md#1-setting-the-file-structure-to-run-mcmctree)). First, we will generate a directory where `MCMCtree` will run wen sampling from the prior:

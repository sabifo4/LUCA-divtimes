...
all_paths   <- c( paste( home_dir, "sum_analyses/01_posterior/GBM/", sep = "" ),
                  paste( home_dir, "sum_analyses/01_posterior/ILN/", sep = "" ) )
#--------------#
# ANALYSE DATA #
#--------------#
...
dataset       <- c( "LUCAdup_fosscb_GBM", "LUCAdup_fosscb_ILN" )
perc          <- perc
def_samples   <- def_samples
prior         <- FALSE
out_dat       <- all_paths
time_unit     <- 1000
out_file_pdf  <- c( "Convergence_plot_post_fosscb_GBM",
                   "Convergence_plot_post_fosscb_ILN" )
out_title_pdf <- c( "GBM","ILN" )
th            <- 0.1
# Generate a list to keep all the returned objects
sum_post_fosscb_QC          <- vector( "list", length( all_paths ) )
names( sum_post_fosscb_QC ) <- dataset
# Now, run the `QC_conv` function in a loop
for( i in 1:length( dataset ) ){
  # NOTE: Only vectors with more than one element rely on `i`
  sum_post_fosscb_QC[[ i ]] <- QC_conv( num_dirs = num_dirs, delcol = delcol[i], 
...
save( file = paste( home_dir, "out_RData/sum_post_fosscb_QC.RData", sep = "" ),
      sum_post_fosscb_QC )

# Print out sum stats in the script as a log

## [[ GBM | conc | fosscb (cbB) ]] ####
sum_post_fosscb_QC$LUCAdup_fosscb_GBM$ESS_results
# $median
# [1] 16119
# 
# $min
...
# $total_samples
# [1] 251153    245
length( sum_post_fosscb_QC$LUCAdup_fosscb_GBM$not_conv_nodes ) # 0
sum_post_fosscb_QC$LUCAdup_fosscb_GBM$num_chains # 16

## [[ ILN | conc | fosscb (cbB) ]] ####
sum_post_fosscb_QC$LUCAdup_fosscb_ILN$ESS_results
# $median
# [1] 17320
# 
# $min
...
# $total_samples
# [1] 268175    245
length( sum_post_fosscb_QC$LUCAdup_fosscb_ILN$not_conv_nodes ) # 0
sum_post_fosscb_QC$LUCAdup_fosscb_GBM$num_chains # 16

...
# GBM: The number of lines is 251153, and so you need to specify one less
# ILN: The number of lines is 268175, and so you need to specify one less
#
...
def_samples <- c( 120005, 251152, 268174 )
...
paths_dat    <- c( paste( home_dir,
                          "sum_analyses/00_prior/mcmc_files_fosscb_CLK",
                          sep = "" ),
                   paste( home_dir,
                          "sum_analyses/01_posterior/mcmc_files_fosscb_GBM",
                          sep = "" ),
                   paste( home_dir,
                          "sum_analyses/01_posterior/mcmc_files_fosscb_ILN",
                          sep = "" )
)
...
labs_in_csv <- paste( "t_n", calib_nodes$LUCAdup_fosscb[,2], sep = "" )
# Start plotting!
for( i in 1:length(dup_dat) ){
  ...
  tmp_allB <- as.numeric( stringr::str_split( string = gsub( pattern = "B\\(|\\)", 
                                                             replacement = "",
                                                             x = calib_nodes$LUCAdup_fosscb[tn_ind,3] ),
                                              pattern = "," )[[1]] )
  # Get max/min x and y
  ...
  tmp_name_calib     <- calib_nodes$LUCAdup_fosscb[tmp_ind_name_calib,1]
